...
long_series <- generate_a_series(intervals=1:50, mu=mu, lambda=lambda + .2, kappa=kappa, seed=1)

long_series2 <- generate_a_series(intervals=1:50, mu=mu, lambda=lambda, kappa=kappa, seed=2)

long_series3 <- generate_a_series(intervals=1:50, mu=mu, lambda=lambda - .2, kappa=kappa, seed=2)

# two long series
concatenate_two_series <- concatenate_series(list(long_series, long_series2, long_series3))
...
# plot
p_single_series <- plot_posteriors(single_series_results, sim_value = .5)
...
# save
save(two_diff_series_samples, file="two_diff_series_samples")
...
two_diff_series_plot <- ggplot(two_diff_series_results, aes(x=length, y=mode, group=group, color=group)) + 
  ...
  labs(y="Estimate", x="Length", title="Lambda estimates with 50% error bars vs. series length") + theme_bw() + scale_color_manual(values=c("#f1a340", "#998ec3")) + scale_y_continuous(limits = c(0, 7))
...
p_single_series2 <- plot_posteriors(single_series2_results, sim_value = .3)
...
# plot posteriors w simulation value
p_single_series3 <- plot_posteriors(single_series3_results, sim_value = .7)
...
p_diff_0.5 <- ggplot(rbind(single_series_results %>% mutate(group="single"), two_series_results), aes(x=length, y=mode, group=group, color=group)) + 
  geom_errorbar(aes(ymin=lower25, ymax=upper75), width=1) +
  geom_line() +
  geom_point()  + geom_hline(yintercept = sim_value, linetype="dashed", color="black") + labs(y="Estimate", x="Length", title="lambda = 0.5") + theme_bw() + scale_color_manual(values=c("#f1a340", "#998ec3", "black")) + scale_y_continuous(limits = c(0, 7))
