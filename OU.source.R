...
two_series_set <- lapply(seq(from=5, to=100, by = 5), function(x) {
  generate_n_series(n=2, intervals=1:x, sigma=sigma, lambda=lambda, mu=mu, t.df = t.df, fix_mu=mu, fix_kappa_log=fix_kappa_log) %>% concatenate_series()
...
for(i in n_observations) {
  ...
  series <- lapply(n_series, function(x) generate_n_series(n=x, sigma=sigma, mu=mu, lambda=0.1, t.df=t.df, intervals = 1:i, seed = 1, fix_mu = fix_mu, fix_kappa_log = fix_kappa_log) %>% concatenate_series()) %>% set_names(as.character(n_series))
  
  series[[as.character(1)]] <- generate_n_series(n=1, sigma=sigma, mu=mu, lambda=0.1, t.df=t.df, intervals = 1:i, seed = 1, fix_mu = fix_mu, fix_kappa_log = fix_kappa_log)
...
short_series_results_list <- lapply(n_observations, function(x) samples_df_par2(short_series_samples[[as.character(x)]], par='lambda') %>% mutate(obs=x)) %>% set_names(as.character(c(n_observations)))

## Plot
short_series_plots <- do.call(rbind, short_series_results_list) %>% ggplot(aes(x=length, y=mean)) + geom_point() + geom_hline(yintercept = .1, linetype="dashed") + facet_grid(cols = vars(obs)) + theme_bw() + labs(y="Posterior mean", x="Number of series") + scale_x_continuous(breaks = c(1, n_series))
