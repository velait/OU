...
### Single series 
To get an idea how many samples are needed for a reliable inference of a single series we test the model with simulated data with different sample sizes. In Figure `r figs("single_series_lambda_0.5",display="num")` below the posterior means and interquartile ranges (IQRs) are plotted agains length of the series. As expected the estimates converge to the simulation value as sample size increases and with very few observation the estimates are inaccurate. Based on this test around 20 samples are needed for a realiable inference with the given parameter values. 

```{r single_series_plots}
grid.arrange(single_series_plots[[1]])
```

`r figs(name="single_series_plots","Means and IQRs of the posteriors for lambda against length of the series. Dashed line marks the simulation value 0.1.")`


### Hierarchical model: two long series
Here we tested the hierarchical version of the model with two long series. The idea is to study if the estimates become more precise given a replicate of the series. It seems that the additional hierarchy level does not make more educated guesses. 

```{r two_series_comparison_plot}
two_series_comparison_plot
```

`r figs(name="two_series_comparison_plot","Means and IQRs of the posteriors for lambda against length of the series. Series A and B denote the data used in hierarchical model and single refers to the model used in previous chapter.")`



### Hierarchical model: many short series
Here we still increase the number of available series and also vary the amount of observations to assess the effect of larger amounts of replicates. As expected and evident in the previous sections there is improvement in accuracy as series length is increased. However, at least upon visual inspection, additional series seem to make only minor improvements in the model performance. 
...
Running times seem to grow linearly as function of sample size. 
```{r}
